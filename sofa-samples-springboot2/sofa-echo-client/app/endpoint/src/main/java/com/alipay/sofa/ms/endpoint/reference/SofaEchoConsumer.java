...
public class SofaEchoConsumer implements ApplicationContextAware {
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        ...
        BenchmarkService benchmarkService = (BenchmarkService) applicationContext.getBean("benchmarkService"); // get remote service proxy
        new Thread(() -> {
            for (; ; ) {
                try {
                    TimeUnit.SECONDS.sleep(1L);
                    String status1 = benchmarkService.send_1k("Hello world!");
                    logger.info(">>>>>>>> benchmark result: " + status1);
                    System.out.println(">>>>>>>> benchmark result: " + status1);
                } catch (Exception e) {
                    logger.error(">>>>>>>> echo result: " + e.getMessage());
                }
            }

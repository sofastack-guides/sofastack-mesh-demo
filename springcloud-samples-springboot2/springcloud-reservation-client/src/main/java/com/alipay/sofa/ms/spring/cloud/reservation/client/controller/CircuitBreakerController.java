...
import com.alipay.sofa.ms.spring.cloud.reservation.client.dto.*;
import com.alipay.sofa.ms.spring.cloud.reservation.client.service.CircuitBreakerService;
...
public class CircuitBreakerController {
    ...
    private boolean checkCircuitBreakerException(Exception e) {
        ...
        return cause instanceof FeignException && message != null
                && circuitBreakerMessageList.stream().anyMatch(message::contains);
    }

    private boolean checkNormalException(Exception e) {
        Throwable cause = e.getCause();
        return cause instanceof FeignException && cause.getMessage() != null && cause.getMessage().contains("502");
    }

    private boolean checkTimeoutException(Exception e) {
        Throwable cause = e.getCause();
        return cause instanceof FeignException && cause.getMessage() != null && cause.getMessage().contains("504");
    }
    ...
    private ResponseType sendRequest(Long executionTime, RequestType requestType) {
        ...
        try {
            service.getServiceInfo(executionTime, requestType.getType());
            logger.info("正常调用");
        } catch (Exception e) {
